...
from dao.preprocess_log import PrepLog

api_key = "../openai.key"


def chat_with_gpt(message, item_id, prompt:Prompt, model="gpt-3.5-turbo", temperature=0.7, max_tokens=2048):
    import openai
    ...
    plog = PrepLog(item_id, assistant_message, assistant_message2)
    plog.commit()
...
def do_preprocess(prep:Preprocess):
    ...
    responce = chat_with_gpt(message, prep.id, PreprocessPrompt, model="gpt-4")
    print(responce)
